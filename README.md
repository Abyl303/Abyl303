# Hi, I'm Abylaikhan 👋
Java Backend Developer

🔹 Spring Boot • REST • PostgreSQL • JPA • Maven  
🔹 Building backend services and automation tools

---

## 🚀 Tech Stack
Java 17 | Spring Boot | REST | PostgreSQL | Hibernate | Maven | Git

---

## 📌 Featured Projects

### 🚀 [Order Service REST API](https://github.com/Abyl303/order-service-api)
RESTful backend service  
Spring Boot • PostgreSQL • JPA • Maven

### 🚀 [Terea Admin Panel](https://github.com/Abyl303/terea-admin-panel)
Full-stack CRUD admin panel  
Spring Boot • JavaScript • Fetch API

### 🔹 [Java Portfolio](https://github.com/Abyl303/Java_portfolio)
Collection of Java practice projects and backend experiments


---

## 📫 Contacts
LinkedIn: https://linkedin.com/in/abylai-izmakhanov-348189342  
Email: abylaikhanizmakhanov@gmail.com
